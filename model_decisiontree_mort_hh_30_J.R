...
dataset<-dataset[ , !(names(dataset) %in% c("readm_hh_30", 'readm_30', "entr_g", "mort_30"))]
...
for (i in 1:10){
  ...
  mod_rf <- caret::train(mort_hh_30~., data=data_train, method="rpart", trControl = ctrl,metric="Kappa")
...
#ALL
mod_mort_hh_DT <- mod_rf
pred_mort_hh_DT <- pred
