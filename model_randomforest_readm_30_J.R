...
dataset<-dataset[ , !(names(dataset) %in% c("mort_hh_30", 'readm_hh_30', "Episodi", "mort_30"))]
...
for (i in 1:10){
  set.seed(73*i)
  h<-holdout(dataset$readm_30,ratio=.70,mode="stratified")
  data_train <- dataset[h$tr,] # train
  data_test  <- dataset[h$ts,] # test
  
  print(table(data_train$readm_30))
  print(table(data_test$readm_30))
  ...
  mod_rf <- caret::train(readm_30~., data=data_train, method="rf",ntree=500, trControl = ctrl,metric="Kappa")
  ...
  data_test$readm_30<-as.factor(data_test$readm_30)
  cm_under<-confusionMatrix(pred,data_test$readm_30,positive="si")
  #draw_confusion_matrix(cm_under)
  test_roc <- function(model, data) {
    roc(data$readm_30, predict(model, data, type = "prob")[, "si"])
  }
...
#ONLY HDOM
mod_readm_30_hdom <- mod_rf
pred_readm_hh_hdom <- pred
#HDOM + FUNCTIONAL
mod_readm_30_hdom_funct <- mod_rf
pred_readm_30_hdom_funct <- pred
#HDOM + POPULATION
mod_readm_30_hdom_pop <- mod_rf
pred_readm_30_hdom_pop <- pred
#ALL
mod_readm_30 <- mod_rf
pred_readm_30 <- pred

plot_readm <- varImp(mod_readm_30)
plot_readm <- plot_readm[["importance"]]

df.plot_readm <- plot_readm %>%
  arrange(desc(Overall)) %>%
  slice(1:20)
names <- c("RDW", "GMA", "charlson", "age", "p_state", "pills_day", "Lym", "Cr", "m_state", "Hb", "Leu",
           "vis_nurs", "Na", "adm_pre", "BMI", "Glu", "days_HH", "K", "N_diag", "barthel")
df.plot_readm <- cbind(names, df.plot_readm)
space <- c(0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4)
par(mfrow=c(1,2))
### HORIZONTAL PLOT
df.plot_readm <- df.plot_readm[seq(dim(df.plot_readm)[1],1),] #rotate for horizontal plot
par(mar = c(4.5, 4.5, 2.5, 1.5)) #bottom,left, top, right
bp <- barplot(height = df.plot_readm$Overall, horiz = TRUE, las = 1, space=space,
              main = "RM3 - 20 Most Important Variables", xlab = "Percentage")
rounded <- round(df.plot_readm$Overall, 2)
text(x = df.plot_readm$Overall, y = bp, label = rounded, pos = 2, cex = 1, col = "black")
yax <- axis(2, at=bp, labels=df.plot_readm$names, tick = FALSE, las=2, 
            line=-1, cex.axis = 0.8)
mean(results$AUC)
...
### ROC Plot
roc.readm_30 <- roc(data_test$readm_30, predict(mod_readm_30, data_test, type = "prob")[, "si"])
lines(roc.readm_30, col = "red")
roc.hdom <- roc(data_test$readm_30, predict(mod_readm_30_hdom, data_test, type = "prob")[, "si"])
plot(roc.hdom, col = "blue")
roc.funct <- roc(data_test$readm_30, predict(mod_readm_30_hdom_funct, data_test, type = "prob")[, "si"])
lines(roc.funct, col = "magenta")
roc.pop <- roc(data_test$readm_30, predict(mod_readm_30_hdom_pop, data_test, type = "prob")[, "si"])
lines(roc.pop, col = "goldenrod4")
title(main = "RM3", line = 2.5)
legend(0.25, 0.5, legend = c("Clinical", "Clinical+Biological","Clinical+Population","All data"),
       col = c("blue", "yellow", "green", "red"), lty = 1:1, cex = 0.75, box.lty = 0)

# 3 ALGORITHMS
plot(roc.readm_30, col = "red")
roc.rf_readm_DT <- roc(data_test$readm_30, predict(mod_readm_30_DT, data_test, type = "prob")[, "si"])
lines(roc.rf_readm_DT, col = "blue")
roc.mod_readm_LR <- roc(data_test$readm_30, predict(mod_readm_30_LR, data_test, type = "prob")[, "si"])
lines(roc.mod_readm_LR, col = "goldenrod4")
