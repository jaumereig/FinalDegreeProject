...
dataset<-dataset[ , !(names(dataset) %in% c("readm_hh_30", 'readm_30', "Episodi", "mort_30"))]
...
for (i in 1:10){
  set.seed(73*i)
  h<-holdout(dataset$mort_hh_30,ratio=.70,mode="stratified")
  data_train <- dataset[h$tr,] # train
  data_test  <- dataset[h$ts,] # test
  
  print(table(data_train$mort_hh_30))
  print(table(data_test$mort_hh_30))
  ...
  #mod_rf <- caret::train(mort_hh_30~., data=data_train, method="rf", trControl = ctrl, metric="Kappa")
  mod_rf <- caret::train(mort_hh_30~., data=data_train, method="rf",ntree=500, trControl = ctrl,metric="Kappa")
  ...
  data_test$mort_hh_30<-as.factor(data_test$mort_hh_30)
  cm_under<-confusionMatrix(pred,data_test$mort_hh_30,positive="si")
  #draw_confusion_matrix(cm_under)
  test_roc <- function(model, data) {
    roc(data$mort_hh_30, predict(model, data, type = "prob")[, "si"])
  }
...
#ONLY HDOM
mod_mort_hh_hdom <- mod_rf
pred_mort_hh_hdom <- pred
#HDOM + FUNCTIONAL
mod_mort_hh_hdom_funct <- mod_rf
pred_mort_hhhdom_funct <- pred
#HDOM + POPULATION
mod_mort_hh_hdom_pop <- mod_rf
pred_mort_hh_hdom_pop <- pred
#ALL
mod_mort_hh <- mod_rf
pred_mort_hh <- pred


plot_mort_hh <- varImp(mod_mort_hh)
plot_mort_hh <- plot_mort_hh[["importance"]]

df.plot_mort_hh <- plot_mort_hh %>%
  arrange(desc(Overall)) %>%
  slice(1:20)
names <- c("barthel", "walk_si", "m_state", "year_cat", "RDW", "Na", "p_state", "Leu", 
           "Lym", "charlson", "GMA", "BMI", "Hb", "age", "Cr",
           "Glu", "dis_circulatory", "K", "follow_pre_nadie", "ox_pre_si")
df.plot_mort_hh <- cbind(names, df.plot_mort_hh)
space <- c(0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4, 0.4)
#par(mfrow=c(1,2))
### HORIZONTAL PLOT
df.plot_mort_hh <- df.plot_mort_hh[seq(dim(df.plot_mort_hh)[1],1),] #rotate for horizontal plot
par(mar = c(4.5, 4.5, 2.5, 1.5)) #bottom,left, top, right
bp <- barplot(height = df.plot_mort_hh$Overall, horiz = TRUE, las = 1, space=space,
              main = "RM2 - 20 Most Important Variables", xlab = "Percentage")
rounded <- round(df.plot_mort_hh$Overall, 2)
text(x = df.plot_mort_hh$Overall, y = bp, label = rounded, pos = 2, cex = 1, col = "black")
yax <- axis(2, at=bp, labels=df.plot_mort_hh$names, tick = FALSE, las=2, 
            line=-1, cex.axis = 0.8)
...
#par(mfrow=c(1,1))
roc.rf_mort_hh <- roc(data_test$mort_hh_30, predict(mod_mort_hh, data_test, type = "prob")[, "si"])
lines(roc.rf_mort_hh, col = "red")
roc._mort_hh_hdom <- roc(data_test$mort_hh_30, predict(mod_mort_hh_hdom, data_test, type = "prob")[, "si"])
plot(roc._mort_hh_hdom, col = "blue")
roc.funct_mort_hh_30 <- roc(data_test$mort_hh_30, predict(mod_mort_hh_hdom_funct, data_test, type = "prob")[, "si"])
lines(roc.funct_mort_hh_30, col = "magenta")
roc.pop_mort_hh_30 <- roc(data_test$mort_hh_30, predict(mod_mort_hh_hdom_pop, data_test, type = "prob")[, "si"])
lines(roc.pop_mort_hh_30, col = "goldenrod4")
title(main = "RM2", line = 2.5)
legend(0.35, 0.65, legend = c("Clinical", "Clinical + Biological","Clinical+Population","All data"),
       col = c("blue", "magenta", "green", "red"), lty = 1:1, cex = 0.5, box.lty = 0)
# 3 ALGORITHMS
plot(roc.rf_mort_hh, col = "red")
roc.rf_mort_hh_DT <- roc(data_test$mort_hh_30, predict(mod_mort_hh_DT, data_test, type = "prob")[, "si"])
lines(roc.rf_mort_hh_DT, col = "blue")
roc.mod_mort_hh_LR <- roc(data_test$mort_hh_30, predict(mod_mort_hh_LR, data_test, type = "prob")[, "si"])
lines(roc.mod_mort_hh_LR, col = "goldenrod4")
